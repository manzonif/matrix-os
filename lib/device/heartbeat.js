...
module.exports = {
  ...
  beat: function() {
    ...
    if ( h++ % 10 === 0){
      console.log('hb:', 'sen>', Matrix.activeSensors, 'svc>', Matrix.activeServices);
    }
    _.each(d.sensors, function(s) {
      ...
      } else if (Matrix.activeSensors.indexOf(s.toLowerCase()) === -1) {
        console.warn(s, 'sensor is requested by the app, but not active');
      } else {
    ...
    if (services.length > 0) {
      ...
      } else if (Matrix.device.service.isRecognition(d.services[0])) {
        Matrix.device.drivers.recognition.ping();
      }
    }
  },
  stop: function() {
